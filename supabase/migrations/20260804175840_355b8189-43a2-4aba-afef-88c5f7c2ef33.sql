...
RETURNS TABLE(user_id uuid, full_name text, efin_tag text, avatar_url text, email_masked text, account_number text)
LANGUAGE plpgsql
...
REVOKE ALL ON FUNCTION public.search_efin_recipients(text) FROM public;
...
RETURNS TABLE(user_id uuid, full_name text, efin_tag text, avatar_url text, email text, account_number text, last_sent_at timestamptz)
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $function$
...
REVOKE ALL ON FUNCTION public.recent_efin_recipients() FROM public;
GRANT EXECUTE ON FUNCTION public.recent_efin_recipients() TO authenticated;