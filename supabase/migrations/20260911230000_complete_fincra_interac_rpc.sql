-- Let the signed-in customer record their bank Interac confirmation number
-- (e.g. CAh9ECkx) without going through collection create.
...
REVOKE ALL ON FUNCTION public.complete_fincra_interac_etransfer(uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.complete_fincra_interac_etransfer(uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.complete_fincra_interac_etransfer(uuid, text) TO service_role;
