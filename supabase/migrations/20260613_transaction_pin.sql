-- Transaction PIN: secure 4-digit PIN required to authorize outgoing transfers.
-- PIN is never stored in plaintext. We hash it with pgcrypto bcrypt (gen_salt('bf')).
...
-- Whether the current user has set a transaction PIN.
...
as $$
  select coalesce(
    (select transaction_pin_hash is not null from public.profiles where id = auth.uid()),
    false
  );
$$;

-- Set / change the current user's transaction PIN.
-- p_pin must be a 4-digit numeric string. Returns true on success.
...
-- Verify a PIN for the current user. Implements a simple lockout:
-- after 5 wrong attempts the PIN is locked for 15 minutes.
-- Returns jsonb: { ok: boolean, locked: boolean, attempts_left: int, locked_until: timestamptz }
