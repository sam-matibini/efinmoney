...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: rl } = await supabase.rpc("check_rate_limit", {
      p_key: `flw_va:${userId}`, p_max_requests: 5, p_window_seconds: 60,
    });
    if (rl === false) {
    ...
    if (existing) {
      return new Response(JSON.stringify({ virtual_account: existing }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    const FLW_KEY = Deno.env.get("FLW_SECRET_KEY");
    if (!FLW_KEY) {
      return new Response(JSON.stringify({ error: "Flutterwave not configured" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    const txRef = `efm_va_${userId.slice(0, 8)}_${Date.now()}`;
    ...
    if (!flwRes.ok || flwJson?.status !== "success") {
      console.error("FLW VA create failed", flwJson);
      return new Response(JSON.stringify({ error: flwJson?.message || "Failed to create virtual account" }), { status: 502, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    const d = flwJson.data;
    const { data: inserted, error: insErr } = await admin.from("virtual_accounts").insert({
      ...
      account_name: d.note || `${profile.first_name || ""} ${profile.last_name || ""}`.trim() || profile.email,
      flw_order_ref: d.order_ref,
      flw_response: d,
      is_permanent: isPermanent,
    ...
    console.error("flw-create-virtual-account error", err);
