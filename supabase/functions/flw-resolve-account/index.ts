...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!res.ok || json?.status !== "success") {
      return new Response(JSON.stringify({ resolved: false, error: json?.message || "Could not resolve account" }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    return new Response(JSON.stringify({ resolved: true, account_name: json.data.account_name, account_number: json.data.account_number }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (err) {
