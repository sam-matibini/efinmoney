/**
 * Credit wallet after Bambora / Worldline payment approval.
 */
...
const BAMBORA_SETTLEMENT_BY_CURRENCY: Record<string, string> = {
  CAD: "1364",
  USD: "1365",
};
...
async function resolveSettlement(admin: Admin, currency: string) {
  const code = BAMBORA_SETTLEMENT_BY_CURRENCY[currency];
  if (code) {
  ...
  const { data } = await admin.from("ledger_accounts")
    ...
    .ilike("name", "Bambora Settlement%")
    .limit(1)
    .maybeSingle();
  if (data?.id) return data;
  if (currency === "CAD") {
    const { data: stripeCad } = await admin.from("ledger_accounts").select("id").eq("code", "1242").maybeSingle();
    if (stripeCad?.id) return stripeCad;
  }
  return null;
}
...
export async function creditWalletViaBambora(
  admin: Admin,
  userId: string,
  currency: string,
  amount: number,
  ...
  desc = "Top-up via Bambora",
): Promise<{ wallet_id: string; already: boolean }> {
  const ccy = currency.toUpperCase();
  const ref = String(idempotencyRef || "").trim();
  if (!ref) throw new Error("Missing Bambora top-up reference");
  if (!Number.isFinite(amount) || !(amount > 0)) {
    throw new Error(`Invalid Bambora credit amount: ${amount}`);
  }
  ...
  const { data: existing } = await admin.from("ledger_entries").select("id")
    .eq("reference_type", "bambora_topup").eq("external_reference", ref).limit(1);
  ...
  if (!asset || !liab) {
    throw new Error(
      `Missing Bambora ledger accounts for ${ccy}` +
        ` (settlement=${BAMBORA_SETTLEMENT_BY_CURRENCY[ccy] || "?"}, liability=${LIABILITY_BY_CURRENCY[ccy] || "?"})`,
    );
  }
  ...
  const { error } = await admin.from("ledger_entries").insert([
    {
      ...
      reference_type: "bambora_topup", external_reference: ref, created_by: userId,
    },
    {
