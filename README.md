...
- **Bank-to-bank methods** — Withdraw and same-company sends pick the payment method the destination country supports: **EFT**, **Interac e-Transfer**, or **wire** (Canada CAD via Nomba); **ACH** or **wire** (US); **Faster Payments** or **wire** (UK); **SEPA** or **SWIFT wire** (euro); local NUBAN / bank credit (NG/GH/KE via Fincra then Nomba). Interac Autodeposit email may be personal or business (including the profile login) or a Canadian mobile. CAD payouts stay Nomba (never Fincra payout, never M-Pesa).
