  ...
  return (
    <motion.div
      ...
      className={`relative overflow-hidden rounded-2xl p-6 ${
        isMain 
          ? 'gradient-primary shadow-glow min-h-[180px]' 
          : 'glass shadow-card'
      }`}
    >
      {isMain && (
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <span className={`font-display font-semibold ${isMain ? 'text-primary-foreground' : 'text-foreground'}`}>
              {currency}
            </span>
          </div>
          <button className={`p-2 rounded-full transition-colors ${
            ...
            <MoreHorizontal className={`w-5 h-5 ${isMain ? 'text-primary-foreground/70' : 'text-muted-foreground'}`} />
          </button>
        </div>

        <div className="mb-4">
          <p className={`text-sm mb-1 ${isMain ? 'text-primary-foreground/70' : 'text-muted-foreground'}`}>
            Available Balance
          </p>
