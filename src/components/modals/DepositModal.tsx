...
const DepositModal = ({ children }: DepositModalProps) => {
  ...
  // Deposit reference uses wallet id + user id prefix
  const reference = selectedWallet
    ? `EFM-${selectedWallet.wallet_id.slice(0, 8).toUpperCase()}`
    : '';
  ...
  const accountNumber = selectedWallet
    ? `**** **** ${selectedWallet.wallet_id.slice(-4).toUpperCase()}`
    : '';
