...
export const useCardMutations = () => {
  ...
  const createCard = useMutation({
    mutationFn: async (input: {
      ...
      if (!isExternal) {
        ...
        const { data, error } = await supabase.functions.invoke('virtual-card-ops', {
          body: {
            action: 'create',
        ...
        const res = data as { error?: string; card?: Card; pan?: string; cvv?: string };
  ...
  const revealCardSecrets = useMutation({
    mutationFn: async (input: { card_id: string; pin: string }): Promise<RevealedCardSecrets> => {
      const { data, error } = await supabase.functions.invoke('virtual-card-ops', {
        body: { action: 'reveal', card_id: input.card_id, pin: input.pin },
      });
      if (error) throw new Error(error.message);
      const res = data as RevealedCardSecrets & { error?: string; has_secrets?: boolean };
      if (res.error) throw new Error(res.error);
