...
export const NOMBA_PAY_CURRENCIES = [...NOMBA_NIGERIA_CURRENCIES, ...NOMBA_INTERNATIONAL_CURRENCIES];
...
export function nombaGatewayLabel(currency: string): string {
  return NOMBA_INTERNATIONAL_CURRENCIES.includes(currency.toUpperCase())
    ? "Nomba (International)"
    : "Nomba (Nigeria)";
}